...
const SeoSidebarWidget = () => {
  ...
  const extractContentFromValues = (vals: Record<string, unknown>): string => {
    let text = '';
    const fieldsToTry = ['content', 'body', 'description', 'text', 'article', 'summary'];
    ...
    for (const field of fieldsToTry) {
      if (vals[field]) {
        if (typeof vals[field] === 'string') {
          ...
          try {
            // Strapi 5 Blocks are arrays of objects
            text += JSON.stringify(vals[field]) + '\n\n';
          } catch (e) {}
        }
      }
    }
  ...
  const handleGenerateClick = async () => {
    try {
      ...
      if (!contentToAnalyze || contentToAnalyze.length < 20) {
        toggleNotification({
          type: 'warning',
          message: formatMessage({ id: 'seo-gemini.warning.no-content', defaultMessage: 'Not enough content to generate SEO metadata.' }),
        });
        return;
      }
      ...
      if (data?.data) {
        ...
        if (generated.title) {
          onChange({ target: { name: seoPrefix + 'metaTitle', value: generated.title, type: 'string' } } as any);
        }
        if (generated.description) {
          ...
          onChange({ target: { name: seoPrefix + 'metaDescription', value: safeDesc, type: 'string' } } as any);
        }
        if (generated.keywords) {
          const kw = Array.isArray(generated.keywords) ? generated.keywords.join(', ') : generated.keywords;
          onChange({ target: { name: seoPrefix + 'keywords', value: kw, type: 'string' } } as any);
        }
        if (generated.metaRobots) {
          onChange({ target: { name: seoPrefix + 'metaRobots', value: generated.metaRobots, type: 'string' } } as any);
        }
        if (generated.structuredData) {
          // Format as JSON object for Strapi
          const sdData = typeof generated.structuredData === 'string' 
            ? JSON.parse(generated.structuredData) 
            : generated.structuredData;
          onChange({ target: { name: seoPrefix + 'structuredData', value: sdData, type: 'json' } } as any);
        }
        
        toggleNotification({
          type: 'success',
          message: formatMessage({ id: 'seo-gemini.success.generated-applied', defaultMessage: 'SEO metadata generated and applied!' }),
        });
      }
    } catch (err: unknown) {
  ...
  return (
    ...
    >
      <Flex direction="column" alignItems="stretch" gap={3}>
        ...
        {generatedData && (
          <Box background="success100" padding={3} hasRadius borderStyle="dashed" borderWidth="1px" borderColor="success200">
            <Flex direction="column" gap={2} alignItems="stretch">
              <Flex gap={2}>
                <Check width={12} height={12} fill="success600" />
                <Typography variant="pi" fontWeight="bold" textColor="success600">Applied to form</Typography>
              </Flex>
              <Box>
                <Typography variant="pi" textColor="neutral700" fontWeight="bold">T:</Typography>{' '}
                <Typography variant="pi" textColor="neutral700">{generatedData.title}</Typography>
              </Box>
              <Box>
                <Typography variant="pi" textColor="neutral700" fontWeight="bold">D:</Typography>{' '}
